...
export const useGlobalErrorHandler = () => {
  ...
  useEffect(() => {
    ...
    const handleError = (event: ErrorEvent) => {
      ...
      if (event.message.includes('TypeError') || event.message.includes('Cannot read properties')) {
        errorType = 'react_error' // TypeErrors in render are typically React issues
      }
      ...
      if (isInIframe()) {
        sendErrorToParent(errorDetails)
        // Auto-request fix for TypeErrors (common code bugs)
        if (errorType === 'react_error') {
          console.log('[ErrorBoundary] TypeError detected, auto-requesting fix...')
